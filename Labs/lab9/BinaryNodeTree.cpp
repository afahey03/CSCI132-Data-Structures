// Name: Aidan Fahey
// Date: 11/11/22
// Implemented moveValuesUpTree, isSameShape and isSameShapeHelper, and levelOrderTraverse
...
void BinaryNodeTree<ItemType>::levelorderTraverse(void visit(ItemType)) const {
  ...
  ArrayQueue<BinaryNode<ItemType>*> levels;

  levels.enqueue(rootPtr);
  while (!levels.isEmpty()) {
    BinaryNode<ItemType>* q = levels.peekFront();
    cout << q->getItem() << " ";
    levels.dequeue();

    if (q->getLeftChildPtr() != nullptr) {
      levels.enqueue(q->getLeftChildPtr());
    }

    if (q->getRightChildPtr() != nullptr) {
      levels.enqueue(q->getRightChildPtr());
    }
  }
}
...
bool BinaryNodeTree<ItemType>::isSameShapeHelper(const BinaryNode<ItemType>* subtree1,
    const BinaryNode<OtherItemType>* subtree2) const {
      ...
      if (!(isSameShapeHelper(subtree1->getLeftChildPtr(), subtree2->getLeftChildPtr()))) {
        return false;
      }
      if (!(isSameShapeHelper(subtree1->getRightChildPtr(), subtree2->getRightChildPtr()))) {
        return false;
      }
