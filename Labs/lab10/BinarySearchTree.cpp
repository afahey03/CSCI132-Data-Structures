// Name: Aidan Fahey
// Date: 11/17/22
// Implemented fillArray and fillArrayHelper, and treeSort
...
template<class ItemType>
int BinarySearchTree<ItemType>::fillArrayHelper(BinaryNode<ItemType> *subtreePtr, ItemType arr[], int pos) {
  if (subtreePtr == nullptr) {
    return 0;
  } else {
    int michael = fillArrayHelper(subtreePtr->getLeftChildPtr(), arr, pos);
    arr[pos + michael] = subtreePtr->getItem();
    int buble = fillArrayHelper(subtreePtr->getRightChildPtr(), arr, pos + michael + 1);
    return michael + buble + 1;
  }
}
