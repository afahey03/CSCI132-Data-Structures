// Name: Aidan Fahey
// Date: 11/17/22
// Implemented swapSubTreeHelper and isBTSHelper
...
void BinaryNodeTree<ItemType>::swapSubTreeHelper(BinaryNode<ItemType>* subTree) {
    if (subTree == nullptr)
      ...
      swapSubTreeHelper(left);
      swapSubTreeHelper(right);
...
ItemType BinaryNodeTree<ItemType>::getMin(const BinaryNode<ItemType>* subTree) const {
    ...
    ItemType value = subTree->getItem();
    if (subTree->getLeftChildPtr() != nullptr)
        value = min(value, getMin(subTree->getLeftChildPtr()));
    if (subTree->getRightChildPtr() != nullptr)
        value = min(value, getMin(subTree->getRightChildPtr()));
    return value;
}
...
ItemType BinaryNodeTree<ItemType>::getMax(const BinaryNode<ItemType>* subTree) const {
    ...
    ItemType value = subTree->getItem();
    if (subTree->getLeftChildPtr() != nullptr)
        value = max(value, getMax(subTree->getLeftChildPtr()));
    if (subTree->getRightChildPtr() != nullptr)
        value = max(value, getMax(subTree->getRightChildPtr()));
    return value;
}
...
bool BinaryNodeTree<ItemType>::isBSTHelper(const BinaryNode<ItemType>* subtree) const {
  ...
  if (left != nullptr && getMax(left) > subtree->getItem()) {
    return false;
  }
  if (right != nullptr && getMin(right) < subtree->getItem()) {
    return false;
  }
  if (!isBSTHelper(left) || !isBSTHelper(right)) {
    return false;
  }
  return true;
}
