 ...
 * Name: Aidan Fahey
 * Date: 10/30/22
 * Implemented and utilized binary search
 * Date: Spring 2021
 * Course: CSCI 132
 * Assignment: Lab 7
 * Instructor: K. Walsh
...
bool binarySearch(const ArrayList<ItemType>& list, int low, int high, const int& num) {
  if (low <= high) {
    int mid = (low + high) / 2;
    if (list.getEntry(mid) == num) {
      return true;
    }
    if (list.getEntry(mid) > num) {
      return binarySearch(list, low, mid - 1, num);
    }
    if (list.getEntry(mid) < num) {
...
ArrayList<ItemType> computeCommon(const ArrayList<ItemType>& first, const ArrayList<ItemType>& second)
{
  ...
  for (int i = 1; i <= first.getLength(); i++) {
    ...
    if (binarySearch(second, 1, second.getLength(), val) && !binarySearch(result, 1, result.getLength(), val)) {
      result.append(val);
    }
  }
