 ...
 * Name: Aidan Fahey
 * Date: 10/30/22
 * Implemented an O(n) method to compute common
 * Date: Spring 2021
 * Course: CSCI 132
 * Assignment: Lab 7
 * Instructor: K. Walsh
...
ArrayList<ItemType> computeCommon(const ArrayList<ItemType>& first, const ArrayList<ItemType>& second)
{
  ...
  int fingerOne = 1;
  int fingerTwo = 1;
  int lengthOne = first.getLength();
  int lengthTwo = second.getLength();
  while (fingerOne <= lengthOne && fingerTwo <= lengthTwo) {
    ItemType itemOne = first.getEntry(fingerOne);
    ItemType itemTwo = second.getEntry(fingerTwo);
    if (itemOne < itemTwo) {
      fingerOne++;
    }
      ...
      fingerOne++;
      fingerTwo++;
      if ((fingerOne < lengthOne) && (fingerTwo < lengthTwo)) {
        while ((first.getEntry(fingerOne) == itemOne)) {
          fingerOne++;
        }
        while ((second.getEntry(fingerTwo) == itemTwo)) {
          fingerTwo++;
        }
      }
    }
