...
bool interchange(int pos1, int pos2, ListInterface<int>* aList) {
    if (pos1 > aList->getLength() || pos2 > aList->getLength()) {
        return false;
    }
    int temp = aList->replace(pos1, aList->getEntry(pos2));
    aList->replace(pos2, temp);
    return true;
}
...
bool join(ListInterface<int>* list1, ListInterface<int>* list2) {
    for (int i = 1; i <= list1->getLength(); i++) {
        if (!list2->insert(list2->getLength() + 1, list1->getEntry(i))) {
            return false;
        }
    }
    return true;
...
void reverse(ListInterface<int>* aList) {
    int back = aList->getLength();
    for (int i = 1; i <= aList->getLength() / 2; i++) {
        int temp = aList->replace(i, aList->getEntry(back));
        aList->replace(back, temp);
        back--;
    }
    return;
}
...
bool split(const ListInterface<int>* source, ListInterface<int>* oddlist, ListInterface<int>* evenlist) {
    for (int i = 1; i <= source->getLength(); i++) {
        int lengthEven = 0;
        int lengthOdd = 0;
        if (source->getEntry(i) % 2 == 0) {
            if (!(evenlist->insert(lengthOdd + 1, source->getEntry(i)))) {
                return false;
            }
            lengthEven++;
        } else {
            if (!(oddlist->insert(lengthOdd + 1, source->getEntry(i)))) {
                return false;
            }
            lengthOdd++;
        }
...
bool partList(const int& x, const ListInterface<int>* source, ListInterface<int>* list1,
    ...
    for (int i = 1; i <= source->getLength(); i++) {
        ...
        if (source->getEntry(i) <= x) {
            if (!(list1->insert(lengthOdd + 1, source->getEntry(i)))) {
                return false;
            }
            lengthEven++;
        } else {
            if (!(list2->insert(lengthOdd + 1, source->getEntry(i)))) {
                return false;
            }
            lengthOdd++;
        }
